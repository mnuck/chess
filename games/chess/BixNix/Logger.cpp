...
BOOST_LOG_ATTRIBUTE_KEYWORD(p_id, "ProcessID",
                            attributes::current_process_id::value_type);
BOOST_LOG_ATTRIBUTE_KEYWORD(t_id, "ThreadID",
...
BOOST_LOG_GLOBAL_LOGGER_INIT(logger, logger_t) {
  ...
  core::get()->add_sink(add_console_log(
  ...
  if (getenv("LOG_TO_FILE")) {
    core::get()->add_sink(add_file_log(
        keywords::file_name = "bixnix_%5N.log", keywords::target = "Logs",
...
bool SetLogLevel(const unsigned int level) {
  ...
  switch (level) {
    ...
    case 5:
      ...
      break;
    default:
      LOG(fatal) << "Logging level NOT set to " << level
                 << " (not actually fatal)";
      success = false;
      break;
  }
...
ScopeLog::ScopeLog(std::string message) : _message(message) {
  LOG(trace) << "ScopeLog Entering " << _message;
}

ScopeLog::~ScopeLog() { LOG(trace) << "ScopeLog Exiting " << _message; }
